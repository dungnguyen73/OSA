...
/*
 * init_pte - Initialize Page Table Entry
 */
int init_pte(uint32_t *pte,
             int pre,    // present bit - indicate whether current page is present in physical memory
             int fpn,    // frame page number - stores the physical address of the page in memory
             int drt,    // dirty - indicate whether the page has been written to
             int swp,    // swap - indicate whether the page has been swapped out to disk
             int swptyp, // swap type - indicate the type of swap device
             int swpoff) // swap offset - stores the offset of the page in the swap device
{
  if (pre != 0)
  {
    if (swp == 0)
 ...
 * @frm_lst   : return frame list
 * function   : allocate req_pgnum of frame in ram
 */
