...
static int translate(
	...
	for (i = 0; i < trans_table->size; i++) {
		if (trans_table->table[i].v_index == second_lv) {
			/* DO NOTHING HERE. This mem is obsoleted- out of date */
			return 1;
		}
	}
	return 0;	
...
addr_t alloc_mem(uint32_t size, struct pcb_t * proc) {
	...
	//check physical addr availble space (maybe not continuous)
	int freespace = 0;
	for(int i=0; i< NUM_PAGES; i++){
		if(num_pages == freespace){
			mem_avail = 1;
			break;
		}
		else{
			if(_mem_stat[i].proc ==0){ //whether this page has not been used by a process.
				freespace++;
			}
		}
	}
	//check virtual address space
	//the virtual address space of a process = its number of page * size of a page
	if( proc->bp + num_pages* PAGE_SIZE > (1<<ADDRESS_SIZE)){  //=>the maximum virtual address spcae
		mem_avail =0;
	}
